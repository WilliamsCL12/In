...
document.getElementById('uploadButton').addEventListener('click', () => {S
...
document.getElementById('calculateButton').addEventListener('click', () => {
    const colorPages = parseInt(document.getElementById('manualPagesInputcolor').value) || 0;
    const bwPages = parseInt(document.getElementById('manualPagesInputbyn').value) || 0;
    ...
    let bwPricePerPage = 0; 
    if (isDoubleSided) {
        if (totalPrintedPages <= 29) {
            pricePerPage = 1.50;
        } else if (totalPrintedPages <= 50) {
            pricePerPage = 1.00;
        } else if (totalPrintedPages <= 99) {
            pricePerPage = 0.90;
        } else if (totalPrintedPages <= 499) {
            pricePerPage = 0.40;
        } else {
            pricePerPage = 0.25;
        }
    ...
    const totalColorPrice = colorPages * colorPricePerPage;
    const totalBWPrice = bwPages * bwPricePerPage;
    const totalPrice = totalColorPrice + totalBWPrice;
